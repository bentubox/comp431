...
describe('Particle Functionality', () => {
    ...
    it('should update the velocity by the acceleration', () => {
        ...
        const p = particle({ position: [1, 1], velocity: [0.5, -0.5], acceleration: [0.1, 0.1] })
        const { velocity } = update(p, 2.0)
        expect(velocity).to.eql([0.7, -0.3])
    })
    ...
    it('particles should wrap around the world', () => {
        ...
        // canvas = { width, height }
        const p = particle({ position: [801, 300], velocity: [0.5, -0.5], acceleration: [0.1, 0.1] })
        const c = {width: 800, height: 600}
        const { position } = update(p, 1.0, c)
        expect(position[0]).gt(0)
        expect(position[0]).lt(c.width)
        expect(position[1]).gt(0)
        expect(position[1]).lt(c.height)
    })
